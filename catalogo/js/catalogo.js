(function(){
    ...
    list.forEach(({ product, qty: amount }) => {
      ...
      item.innerHTML = `
        ...
        <div>
          ...
          <span>${safe(product.presentation)}<br>x ${amount} - ${product.price == null ? "Consultar" : money(product.price * amount)}</span>
        </div>
  ...
  function syncCustomerFrom(source){
    ...
    const changed = name !== state.customerName || address !== state.deliveryAddress;
  ...
  function unlockDiscount(){
    const pin = prompt("PIN interno para aplicar descuento");
    if(pin !== cfg.discountPin){
      alert("PIN incorrecto.");
      return;
    }
    $("#discountBoxMobile").classList.remove("is-hidden");
    $("#discountBoxDesktop").classList.remove("is-hidden");
  }
  ...
  function createPdfBlob(){
    ...
    content += pdfText(356, y, 8.5, "CANT", true);
    content += pdfText(405, y, 8.5, "PRECIO", true);
    content += pdfText(485, y, 8.5, "IMPORTE", true);
    content += pdfLine(40, y - 8, 555, y - 8);
    y -= 24;
    ...
    list.forEach(({ product, qty }) => {
      ...
      content += pdfText(112, y, 8, `${product.name} - ${product.presentation}`.slice(0, 58), false);
      content += pdfText(365, y, 8, String(qty), false);
      content += pdfText(405, y, 8, product.price == null ? "Consultar" : money(price), false);
      content += pdfText(485, y, 8, product.price == null ? "Consultar" : money(price * qty), false);
      y -= 18;
    });
    ...
    y -= 36;
    (cfg.quoteDisclaimer.match(/.{1,96}(\s|$)/g) || [cfg.quoteDisclaimer]).forEach(line => {
      if(y < 60) newPage();
  ...
  function buildMessage(){
    ...
    quoteList().forEach(({ product, qty }) => {
      ...
      lines.push(`- ${product.code} | ${product.name} (${product.presentation}) | Cant: ${qty} | Precio: ${price} | Importe: ${amount}`);
    });
  ...
  function generatePdfOnly(){
    ...
    try{
      ...
      const filename = `presupuesto-barretodo-${new Date().toISOString().slice(0,10)}.pdf`;
  ...
  function bind(){
    ...
    $("#discountMobile").addEventListener("click", unlockDiscount);
    $("#discountDesktop").addEventListener("click", unlockDiscount);
