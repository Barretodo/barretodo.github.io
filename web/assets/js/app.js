...
(function(){
  ...
  async function loadCats(){
    const paths=['data/categories.json','../data/categories.json','/data/categories.json']; let data=null;
    for(const u of paths){ try{ const r=await fetch(u,{cache:'no-cache'}); if(r.ok){ data=await r.json(); break; } }catch(e){} }
    if(!data) return;
    const wrap=document.querySelector('#cats')||document.querySelector('#rubros-grid'); if(!wrap) return;
    wrap.innerHTML='';
    data.forEach(cat=>{
      const el=document.createElement('div'); el.className='card';
      el.innerHTML=`<img class="thumb" src="${cat.img}" alt="${cat.name}"><div class="body"><h3 class="title">${cat.name}</h3><p class="muted">${cat.desc}</p></div>`;
      el.onclick=()=>openModal(cat); wrap.appendChild(el);
    });
  }
